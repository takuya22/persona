...
moderator_review_agent = LlmAgent(
    name="ModeratorAgent",
    model=GEMINI_MODEL,
    instruction="""あなたは3つのペルソナレビュアーにグループインタビューを実施するモデレーターです。レビューを依頼してください。レビュアーは
    田中 美紀（34歳、専業主婦）,
    山田 浩（67歳、退職者）,
    田中 優太（20歳、大学生）,
    鈴木 葵（16歳、高校2年生）の4名です。
レビュアー内で3~5往復の議論を行なってください。議論の中で新たな視点を引き出し、レビュー内容を深掘りしてください。""",
    output_key="conversation_summary",
    description="4つのペルソナによる並列レビュー分析を実行"
)

conversation_agent = LoopAgent(
    name="ConversationAgent",
    sub_agents=[
        ALL_REVIEWERS[6], ALL_REVIEWERS[7], ALL_REVIEWERS[8], ALL_REVIEWERS[9]
    ],
    max_iterations=5,
)
...
synthesis_agent = LlmAgent(
    ...
    model=GEMINI_MODEL,
    instruction="""あなたはグループインタビューを統合するAIアシスタントです。
...
### 総合評価
**★[1-5の総合評価] / 5** (平均値: [数値])
[会話を統合した総合的な評価理由を3-4文で]

提供された会話内容のみに基づいて分析し、多様性を活かした包括的で客観的な統合評価を作成してください。
""",
    description="ペルソナのレビューを統合し多角的総合評価を作成",
)


root_agent = SequentialAgent(
    name="PersonaGroupInterviewAgent",
    description="4つのペルソナレビュアーにグループインタビューを実施し、インタビュー結果を収集するエージェント",
    sub_agents=[
        moderator_review_agent,
        conversation_agent,
        synthesis_agent,
    ],
)